...
export default function AvatarMenu({ name, avatarUrl, onLogout }: Props) {
  ...
  useEffect(() => {
    if (me?.id) register(me.id);
    return () => {
      if (me?.id) unregister(me.id);
    };
  }, [me?.id]);
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      try {
        localStorage.setItem("user_avatar", String(json.url || dataUrl));
      } catch {}
          ...
          >
            <AvatarStatus status={status}>
              <AvatarPrimitive.Root
                className="inline-flex h-10 w-10 select-none items-center justify-center overflow-hidden rounded-full align-middle border border-border"
                key={avatar || "fallback"}
              >
                {avatar ? (
                  ...
                  <AvatarPrimitive.Fallback className="flex h-full w-full items-center justify-center bg-primary text-primary-foreground">
                    {initial}
            ...
            {uploading && (
              <span
                aria-live="polite"
                className="absolute inset-0 rounded-full bg-black/30 animate-pulse"
              />
            )}
          </button>
