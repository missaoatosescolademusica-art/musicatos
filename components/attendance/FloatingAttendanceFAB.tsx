...
export default function FloatingAttendanceFAB() {
    ...
    const tick = async () => {
      try {
        ...
        const names = (json.data || [])
          .slice(0, 3)
          .map((x: any) => x.student.fullName);
        const payload = { count, names };
        const last = JSON.parse(localStorage.getItem(key) || "{}");
        if (last.count === count && Date.now() - (last.ts || 0) < CHK_INTERVAL)
          return;
  ...
  function showAlert(payload: { count: number; names: string[] }) {
    ...
    const desc =
      payload.count > 0
        ? `${payload.count} sem marcação`
        : "Nenhum aluno sem marcação";
    ...
    toast(desc, {
      ...
      action:
        payload.count > 0
          ? { label: "Abrir chamada", onClick: () => setOpen(true) }
          : undefined,
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          ...
          className="fixed bottom-6 right-6 z-50 rounded-full h-14 w-14 p-0 bg-primary text-primary-foreground hover:bg-primary/90 shadow-xl focus-visible:ring-[3px]"
        >
          <ClipboardList className="h-6 w-6" />
        </Button>
      </DialogTrigger>
      <DialogContent className="w-[95vw] max-w-5xl bg-card border border-border p-0">
        <div className="p-6">
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground">
              Totais e lista detalhada por categoria
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="mt-6 grid grid-cols-1 gap-4">
            {[
              ...
              {
                title: "Sem marcação",
                items: grouped.unmarked,
                color: "text-foreground",
              },
            ].map((section, idx) => (
              <details
                key={idx}
                className="group bg-card border border-border rounded shadow-sm"
                    ...
                    {section.title !== "Sem marcação" ? (
                      <Button
                        ...
                        onClick={() =>
                          markBatch(
                            section.items,
                            section.title === "Presente" ? "PRESENT" : "ABSENT",
                          )
                        }
                        disabled={loading || section.items.length === 0}
                      >
                        Marcar em lote
                      </Button>
                    ) : (
                      <div className="flex gap-2 flex-wrap">
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => markBatch(section.items, "PRESENT")}
                          disabled={loading || section.items.length === 0}
                        >
                          Todos Presente
                        </Button>
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => markBatch(section.items, "ABSENT")}
                          disabled={loading || section.items.length === 0}
                        >
                          Todos Ausente
                        </Button>
